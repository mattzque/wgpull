...
impl<T: CommandExecutor> Backend for SystemdBackend<T> {
    fn is_compatible(&self) -> bool {
        ...
        if !self.command.service_is_running_and_enabled("systemd-networkd") {
    ...
    fn update_local_state(&self, state: &NodeState) -> Result<bool> {
        ...
        let has_netdev_changed =
            Self::write_if_changed(&netdev_path, netdev_contents.as_ref())?;
        ...
        let has_network_changed =
            Self::write_if_changed(&network_path, network_contents.as_ref())?;

        if (has_netdev_changed || has_network_changed) && self.config.reload_networkd {
            if self.config.delete_interface_before_reload {
                let _ = self.command.delete_wireguard_interface(&self.config.interface);
            }
